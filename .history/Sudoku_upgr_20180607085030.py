...
while True:
    print("Wprowadź 3 liczby w formacie y x z, np. 4 5 8")
    print("y - wiersz")
    print("x - kolumna")
    print("z - wartość \n ")
    ...
    if len(x) != 5: #and str(x[0]) not in numbers and str(x[2]) not in numbers and str(x[5]) not in numbers:
        print("Warunek długość OK")
        continue  

    if str(x[0]) not in numbers:# or str(x[2])  not in numbers or str(x[5]) not in numbers:
        print("BŁĄD - niepoprawny format!\n ")
        continue 
    else:
        print("Warunek długość str(x[0]) - OK ")
